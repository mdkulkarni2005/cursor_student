...
import { GenerateBoqForm } from "@/components/boq-estimator/generate-form";
...
const STATUS_LABEL: Record<string, string> = {
  READY: "Ready", GENERATING: "Estimating", FAILED: "Failed", DRAFT: "Draft", QUEUED: "Queued",
};

export default async function BoqEstimatorPage() {
  ...
  const [items, quota] = await Promise.all([
    prisma.document.findMany({ where: { ownerId: user.id, type: "BRANCH_SOLVER", feature: "boq-estimator" }, orderBy: { updatedAt: "desc" }, take: 20 }),
  ...
  return (
    <AppShell user={await shellUserFrom(user)}>
      <div className="mx-auto max-w-[1100px]">
        <div className="mb-6">
          <h1 className="font-display text-[30px] font-semibold tracking-tight text-ink">BOQ Estimator</h1>
          <p className="mt-1 text-[14px] text-muted">Type your dimensions/scope or upload a drawing and get an itemized Bill of Quantities with rates and a grand total.</p>
        </div>
            ...
            <div className="rounded-2xl border border-line bg-card p-5">
              ...
              {items.length === 0 ? (
                <p className="rounded-xl border border-dashed border-line bg-surface/50 p-5 text-center text-[13px] text-muted">
                  Your generated estimates will appear here.
                </p>
              ) : (
                <div className="space-y-2.5">
                  {items.map((d) => (
                    <Link key={d.id} href={`/boq-estimator/${d.id}`} className="group flex items-center gap-3 rounded-xl border border-line p-3 transition-colors hover:border-cyan/40">
