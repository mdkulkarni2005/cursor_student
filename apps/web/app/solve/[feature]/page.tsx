...
import { SolveBranchForm } from "@/components/branch-solver/solve-form";
...
export default async function BranchSolverPage({ params }: { params: Promise<{ feature: string }> }) {
  ...
  const [items, quota] = await Promise.all([
    prisma.document.findMany({ where: { ownerId: user.id, type: "BRANCH_SOLVER", feature: slug }, orderBy: { updatedAt: "desc" }, take: 20 }),
  ...
  return (
    <AppShell user={await shellUserFrom(user)}>
      <div className="mx-auto max-w-[1100px]">
        <div className="mb-6">
          <h1 className="font-display text-[30px] font-semibold tracking-tight text-ink">{feature.label}</h1>
          <p className="mt-1 text-[14px] text-muted">{feature.blurb}</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
          <div className="min-w-0">
            ...
            <SolveBranchForm feature={feature} />
          </div>
            ...
            <div className="rounded-2xl border border-line bg-card p-5">
              ...
              {items.length === 0 ? (
                ...
                <div className="space-y-2.5">
                  {items.map((d) => (
                    <Link key={d.id} href={`/solve/${slug}/${d.id}`} className="group flex items-center gap-3 rounded-xl border border-line p-3 transition-colors hover:border-cyan/40">
