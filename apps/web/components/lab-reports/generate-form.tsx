...
import { generateLabReportAction, type LabReportFormState } from "@/lib/actions/lab-reports";
...
export function GenerateLabReportForm() {
  const [state, action, pending] = useActionState<LabReportFormState, FormData>(
    generateLabReportAction,
    {},
  );
  ...
  return (
    <form action={action} className="rounded-2xl border border-line bg-card p-5">
      <GeneratingOverlay label="Writing up your lab report…" />
      <div className="mb-4">
        <label htmlFor="readingsText" className={label}>
          Your raw readings
        </label>
        <textarea
          id="readingsText"
          name="readingsText"
          rows={4}
          placeholder="e.g. Experiment: verify Ohm's law. Trial 1: V=2V, I=0.4A. Trial 2: V=4V, I=0.8A…"
      ...
      <div className="mb-4">
        <label htmlFor="image" className={label}>
          …or snap a photo of your observation table / graph
        </label>
        <input
          id="image"
          name="image"
      ...
      <div className="mb-5">
        ...
        <textarea
          ...
          name="instructions"
          rows={2}
          placeholder="e.g. include % error calculation · use your college's lab report format"
          className={`${box} resize-none`}
        />
      </div>
      ...
      >
        {pending ? "Generating…" : "Generate lab report →"}
      </button>
    </form>
  );
}
