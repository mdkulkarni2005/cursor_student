...
export function SolveBranchForm({ feature }: { feature: BranchSolverFeature }) {
  const [state, action, pending] = useActionState<BranchSolverFormState, FormData>(
    solveBranchAction,
    {},
  );
  ...
  return (
    <form action={action} className="rounded-2xl border border-line bg-card p-5">
      <GeneratingOverlay label="Solving…" />
      <input type="hidden" name="feature" value={feature.slug} />
      <div className="mb-4">
      ...
      <div className="mb-5">
        ...
        <textarea
          ...
          name="instructions"
          rows={2}
          placeholder="e.g. show all steps · use SI units · assume standard values where not given"
          className={`${box} resize-none`}
        />
      </div>
