...
export default function AssignmentsList() {
  ...
  const load = useCallback(() => {
    ...
    client
      .listAssignments()
      .then((r) => setDocs(r.assignments))
      .catch((err) => setError(err instanceof Error ? err.message : "Couldn't load assignments."))
      .finally(() => {
        setLoading(false);
        setRefreshing(false);
      });
    ...
    <View style={styles.screen}>
      <Stack.Screen
        options={{
          title: "Assignments",
          headerRight: () => (
            <Pressable onPress={() => router.push("/assignments/new")}>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.intro}>
            <Text style={styles.title}>Assignment Solver</Text>
            <Text style={styles.subtitle}>Upload questions or photos of your handwritten assignments — AI solves them step by step.</Text>
          </View>
        }
        ListEmptyComponent={
          loading ? (
            ...
            <View style={styles.emptyState}>
              <Text style={styles.empty}>Your solved assignments will appear here.</Text>
            </View>
          )
        }
        renderItem={({ item }) => {
            ...
            <Pressable onPress={() => router.push(`/assignments/${item.id}`)}>
              <Card style={styles.row}>
                <View style={styles.rowIconBadge}>
                  <PencilIcon size={16} color={colors.danger} />
                </View>
...
const styles = StyleSheet.create({
  ...
  rowIconBadge: { width: 36, height: 36, borderRadius: radius.lg, backgroundColor: colors.dangerTint, alignItems: "center", justifyContent: "center" },
