...
export default async function PaymentsPage() {
  ...
  return (
    <>
      <div className="mb-6">
        <h1 className="font-display text-[26px] font-bold text-ink">Payments</h1>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        {totals.map((t) => (
          <div key={t.status} className="rounded-2xl border border-line bg-card p-4">
            <p className="mb-1 text-[13px] font-semibold uppercase tracking-wide text-faint">{t.status}</p>
            <p className="font-display text-[22px] font-bold text-ink">₹{((t._sum.amountCents ?? 0) / 100).toFixed(2)}</p>
            <p className="text-[13px] text-faint">{t._count._all} transaction{t._count._all === 1 ? "" : "s"}</p>
          </div>
        ))}
        {totals.length === 0 && (
      ...
      {payments.length > 0 && (
        <div className="overflow-x-auto rounded-2xl border border-line bg-card">
          <table className="w-full text-left text-[15px]">
            <thead>
              <tr className="border-b border-line text-[13px] uppercase tracking-wide text-faint">
            ...
            <tbody>
              {payments.map((p) => (
                <tr key={p.id} className="border-b border-line/60 last:border-0">
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-[13px] font-semibold ${STATUS_STYLE[p.status]}`}>{p.status}</span>
                  </td>
      ...
      <p className="mt-3 text-[13px] text-faint">Captured total shown above: ₹{(capturedTotal / 100).toFixed(2)}.</p>
    </>
  );
}
