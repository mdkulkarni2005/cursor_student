...
export default async function TemplatesPage() {
  ...
  return (
    <>
      <div className="mb-5">
        <h1 className="font-display text-[26px] font-bold text-ink">Templates</h1>
      ...
      <form action={createTemplate} className="mb-5 grid gap-2 rounded-2xl border border-line bg-card p-4 sm:grid-cols-2 lg:grid-cols-5">
        <select name="type" required className="rounded-lg border border-line bg-input px-2.5 py-2 text-[15px] text-ink">
          <option value="REPORT">Report</option>
          <option value="PPT">PPT</option>
          <option value="RESUME">Resume</option>
        </select>
        <select name="institutionId" className="rounded-lg border border-line bg-input px-2.5 py-2 text-[15px] text-ink">
        ...
        <input
          ...
          required
          className="rounded-lg border border-line bg-input px-3 py-2 text-[15px] text-ink placeholder:text-faint"
        />
        <input
          type="file"
          name="file"
          accept=".docx,.pptx"
          required
          className="rounded-lg border border-line bg-input px-2.5 py-1.5 text-[14.5px] text-ink file:mr-2 file:rounded-md file:border-0 file:bg-surface file:px-2 file:py-1 file:text-[13.5px]"
        />
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-1.5 text-[14.5px] text-soft">
            <input type="checkbox" name="isDefault" className="h-3.5 w-3.5" />
            Set as default
          </label>
          <button type="submit" className="rounded-lg bg-cyan px-3 py-2 text-[14.5px] font-semibold text-on-accent hover:opacity-90">
            Upload
          </button>
        </div>
      </form>
            ...
            {templates.map((t) => (
              <tr key={t.id} className="border-b border-line/60 last:border-0 hover:bg-surface">
                ...
                <td className="px-3 py-2.5">
                  <span className={`rounded-full px-2 py-0.5 text-[13px] font-semibold ${TYPE_STYLE[t.type]}`}>{t.type}</span>
                </td>
