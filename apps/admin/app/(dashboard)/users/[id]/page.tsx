...
export default async function UserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <SetBreadcrumb label={user.name ?? user.email} />
      <Link href="/users" className="mb-4 inline-block text-[14.5px] font-medium text-cyan hover:underline">
        ← All users
      </Link>
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="font-display text-[26px] font-bold text-ink">{user.name ?? "Unnamed"}</h1>
            {user.suspended && (
              <span className="rounded-full bg-danger/12 px-2 py-0.5 text-[13px] font-semibold text-danger">
                suspended
              </span>
            )}
          </div>
          ...
          {user.subscription ? (
            <dl className="space-y-1.5 text-[15px]">
              <div className="flex justify-between">
                ...
                <dd>
                  <span className={`rounded-full px-2 py-0.5 text-[13px] font-semibold ${SUB_STATUS_STYLE[user.subscription.status]}`}>
            ...
            <p className="text-[15px] text-faint">No subscription record — user is on {resolvedTierName ?? "the default free tier"} (manual grant / default, no billing history).</p>
          )}
        </div>
      ...
      <div className="mb-6 rounded-2xl border border-line bg-card p-4">
        <p className="mb-3 text-[13px] font-semibold uppercase tracking-wide text-faint">Credit consumption (generations)</p>
          ...
          {["ASSIGNMENT", "REPORT", "PPT"].map((kind) => {
            ...
            return (
              <div key={kind} className="rounded-xl border border-line/60 bg-surface p-3">
                <p className="text-[13px] font-medium text-faint">{kind}</p>
      ...
      <div className="mb-6 rounded-2xl border border-line bg-card p-4">
        <p className="mb-3 text-[13px] font-semibold uppercase tracking-wide text-faint">Cost to us (estimated)</p>
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-xl border border-line/60 bg-surface p-3">
            <p className="text-[13px] font-medium text-faint">AI spend</p>
            <p className="font-display text-[22px] font-bold text-ink">${(cost.aiCostCentsThisMonth / 100).toFixed(2)}</p>
            <p className="text-[13px] text-faint">this month · ${(cost.aiCostCentsAllTime / 100).toFixed(2)} all-time</p>
          </div>
          <div className="rounded-xl border border-line/60 bg-surface p-3">
            <p className="text-[13px] font-medium text-faint">Storage</p>
            <p className="font-display text-[22px] font-bold text-ink">${cost.storageCostUsdPerMonth.toFixed(3)}/mo</p>
            <p className="text-[13px] text-faint">{(cost.storageBytes / 1024 ** 2).toFixed(1)} MB in R2</p>
          </div>
          <div className="rounded-xl border border-line/60 bg-surface p-3">
            <p className="text-[13px] font-medium text-faint">Total est. cost</p>
            <p className="font-display text-[22px] font-bold text-ink">
              ${(cost.aiCostCentsAllTime / 100 + cost.storageCostUsdPerMonth).toFixed(2)}
            </p>
            <p className="text-[13px] text-faint">AI all-time + storage/mo</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-line bg-card p-4">
          <p className="mb-3 text-[13px] font-semibold uppercase tracking-wide text-faint">Recent documents</p>
          {user.documents.length === 0 ? (
            <p className="text-[15px] text-faint">None yet.</p>
          ) : (
            <ul className="space-y-2 text-[15px]">
              {user.documents.map((d) => (
                <li key={d.id} className="flex items-center justify-between border-b border-line/60 pb-2 last:border-0 last:pb-0">
                  ...
                  <span className="ml-2 shrink-0 text-[13px] text-faint">{d.status} · {fmtDateTime(d.createdAt)}</span>
                </li>
              ))}
            </ul>
          )}
        ...
        <div className="rounded-2xl border border-line bg-card p-4">
          <p className="mb-3 text-[13px] font-semibold uppercase tracking-wide text-faint">Recent DSA attempts</p>
            ...
            <ul className="space-y-2 text-[15px]">
              {user.dsaAttempts.map((a) => (
                <li key={a.id} className="flex items-center justify-between border-b border-line/60 pb-2 last:border-0 last:pb-0">
                  ...
                  <span className="ml-2 shrink-0 text-[13px] text-faint">
