  ...
  return (
    <div className="rounded-2xl border border-line bg-card p-4">
      ...
      {error ? (
        ...
        <p className="text-[15px] text-faint">No active sessions.</p>
      ) : (
          ...
          {sessions.map((s) => (
            <li key={s.id} className="flex items-center justify-between gap-3 border-b border-line/60 pb-2 last:border-0 last:pb-0">
              ...
              <button
                ...
                className="shrink-0 rounded-lg border border-danger/40 px-2.5 py-1 text-[13.5px] font-semibold text-danger transition hover:bg-danger/10 disabled:opacity-50"
              >
                {busyId === s.id ? "…" : "Log out"}
              </button>
            </li>
      ...
      {localError && <p className="mt-2 text-[13.5px] text-danger">{localError}</p>}
    </div>
  );
}
