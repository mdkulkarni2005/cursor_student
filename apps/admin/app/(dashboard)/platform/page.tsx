...
export default async function PlatformPage() {
  ...
  return (
    <>
      <div className="mb-6">
        <h1 className="font-display text-[26px] font-bold text-ink">Platform</h1>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-line bg-card p-4">
          <p className="mb-1 text-[13px] font-semibold uppercase tracking-wide text-faint">AI Gateway balance</p>
          {gatewayOk ? (
            <>
            ...
            <p className="text-[15px] text-danger">{(summary.gateway as { error: string }).error}</p>
          )}
        </div>

        <div className="rounded-2xl border border-line bg-card p-4">
          <p className="mb-1 text-[13px] font-semibold uppercase tracking-wide text-faint">AI spend (estimated)</p>
          <p className="font-display text-[24px] font-bold text-ink">{usd(summary.aiSpendCentsThisMonth)}</p>
          <p className="text-[13px] text-faint">this month · {usd(summary.aiSpendCentsAllTime)} all-time</p>
        </div>

        <div className="rounded-2xl border border-line bg-card p-4">
          <p className="mb-1 text-[13px] font-semibold uppercase tracking-wide text-faint">Storage cost (estimated)</p>
          <p className="font-display text-[24px] font-bold text-ink">${summary.storageCostUsdPerMonth.toFixed(2)}/mo</p>
          <p className="text-[13px] text-faint">{(summary.storageBytes / 1024 ** 3).toFixed(2)} GB in R2</p>
        </div>

        <div className="rounded-2xl border border-line bg-card p-4">
          <p className="mb-1 text-[13px] font-semibold uppercase tracking-wide text-faint">Concurrent active users</p>
          <p className="font-display text-[24px] font-bold text-ink">{summary.concurrentActiveUsers}</p>
          <p className="text-[13px] text-faint">seen in the last 5 minutes</p>
        </div>
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-line bg-card p-4">
          <p className="mb-2 text-[13px] font-semibold uppercase tracking-wide text-faint">Users</p>
          <p className="font-display text-[22px] font-bold text-ink">{totalUsers}</p>
          <p className="text-[13px] text-faint">total registered</p>
        </div>
        <div className="rounded-2xl border border-line bg-card p-4">
          <p className="mb-2 text-[13px] font-semibold uppercase tracking-wide text-faint">Active subscriptions</p>
          <p className="font-display text-[22px] font-bold text-ink">{activeSubs}</p>
          <p className="text-[13px] text-faint">status = ACTIVE</p>
        </div>
      </div>
