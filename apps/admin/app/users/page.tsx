      ...
      <div className="mb-5 flex items-center justify-between gap-4">
        ...
        <form className="w-full max-w-[320px]">
          <input
            ...
            className="w-full rounded-lg border border-line bg-input px-3 py-2 text-[13px] text-ink placeholder:text-faint focus:outline-none focus:ring-2 focus:ring-cyan/30"
          />
        </form>
      </div>
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-center gap-2 text-[12.5px]">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Link
              key={p}
              href={`/users?${new URLSearchParams({ ...(query ? { q: query } : {}), page: String(p) })}`}
