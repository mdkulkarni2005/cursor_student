...
function QuotaField({ name, label, value }: { name: string; label: string; value: number | null | undefined }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="w-48 shrink-0 text-[12.5px] text-soft">{label}</span>
      <input
        type="number"
        min={0}
        name={name}
        ...
        disabled={unlimited}
        className="w-24 rounded-lg border border-line bg-input px-2 py-1.5 text-[13px] text-ink disabled:opacity-40"
      />
  ...
  return (
    <form action={action} className="space-y-5 rounded-2xl border border-line bg-card p-5">
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div>
          <label className="mb-1 block text-[11.5px] font-semibold uppercase tracking-wide text-faint">Slug</label>
          <input
            name="slug"
            defaultValue={initial?.slug}
            required
            placeholder="pro"
            className="w-full rounded-lg border border-line bg-input px-3 py-2 text-[13px] text-ink placeholder:text-faint"
          />
        </div>
        <div>
          <label className="mb-1 block text-[11.5px] font-semibold uppercase tracking-wide text-faint">Name</label>
          <input
            name="name"
        ...
        <div>
          <label className="mb-1 block text-[11.5px] font-semibold uppercase tracking-wide text-faint">Sort order</label>
          <input
            type="number"
            name="sortOrder"
            defaultValue={initial?.sortOrder ?? 0}
            className="w-full rounded-lg border border-line bg-input px-3 py-2 text-[13px] text-ink"
          />
        </div>
        <div className="sm:col-span-2">
          <label className="mb-1 block text-[11.5px] font-semibold uppercase tracking-wide text-faint">Description</label>
          <input
            name="description"
        ...
        <div>
          <label className="mb-1 block text-[11.5px] font-semibold uppercase tracking-wide text-faint">Price (₹ / period)</label>
          <input
            type="number"
            min={0}
            step={0.01}
        ...
        <div>
          <label className="mb-1 block text-[11.5px] font-semibold uppercase tracking-wide text-faint">Billing period</label>
          <select
            name="billingPeriod"
        ...
        <div>
          <label className="mb-1 block text-[11.5px] font-semibold uppercase tracking-wide text-faint">Currency</label>
          <input
            name="currency"
        ...
        <div>
          <label className="mb-1 block text-[11.5px] font-semibold uppercase tracking-wide text-faint">Trial days (this tier)</label>
          <input
            type="number"
            min={0}
            name="trialDays"
      ...
      <div>
        ...
        <div className="space-y-2">
          {FEATURE_KEYS.map((key) => (
            <label key={key} className="flex items-center gap-2 text-[12.5px] text-soft">
