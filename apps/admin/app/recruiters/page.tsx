...
export default async function RecruitersPage() {
  ...
  return (
    <AdminShell>
      <div className="mb-5">
        <h1 className="font-display text-[24px] font-bold text-ink">Recruiters</h1>
            ...
            {recruiters.map((r) => (
              <tr key={r.id} className="border-b border-line/60 last:border-0 align-top hover:bg-surface">
                ...
                <td className="px-3 py-2.5">
                  ...
                  {r.status === "REJECTED" && r.rejectionNote && <p className="mt-1 text-[11px] text-faint">{r.rejectionNote}</p>}
                </td>
